...
propose!{D<:ProposalDensity,S<:MCSample}(d::D,v::Vector{S}) = (@simd for j=1:length(v) @inbounds propose!(d,v[j]) end ; v)
propose!{D<:ProposalDensity,S<:MCSample}(d::Vector{D},v::Vector{S}) = (@simd for j=1:length(v) @inbounds propose!(d[j],v[j]) end ; v)
